...
import (
	...
	errors "github.com/Juniper/apstra-go-sdk/errors"
)
...
func (c Client) GetTemplateRackBased2(ctx context.Context, id string) (design.TemplateRackBased, error) {
	...
	if response == nil {
		return design.TemplateRackBased{}, errors.Internal("template is unexpectedly nil")
	}

	if response.TemplateType() != enum.TemplateTypeRackBased {
		return design.TemplateRackBased{}, errors.WrongType(fmt.Sprintf("template with id %q has wrong type: expected %q got %q", id, enum.TemplateTypeRackBased, response.TemplateType()))
	}
	...
	return design.TemplateRackBased{}, errors.Internal(fmt.Sprintf("response has unexpected underlying type %T", response))
}
...
func (c Client) GetTemplatesRackBased2(ctx context.Context) ([]design.TemplateRackBased, error) {
	...
	for i, t := range templates {
		...
		if !ok {
			return nil, errors.Internal(fmt.Sprintf("template at index %d claims to be a %q but has type %T", i, enum.TemplateTypeRackBased, t))
		}
		if tt == nil {
...
func (c Client) GetTemplateRackBasedByLabel2(ctx context.Context, label string) (design.TemplateRackBased, error) {
	...
	if t == nil {
		return design.TemplateRackBased{}, errors.Internal(fmt.Sprintf("template with label %q is unexpectedly nil", label))
	}

	if t.TemplateType() != enum.TemplateTypeRackBased {
		return design.TemplateRackBased{}, errors.WrongType(fmt.Sprintf("template with label %q has type %q", label, t.TemplateType()))
	}

	result, ok := t.(*design.TemplateRackBased)
	if !ok {
		return design.TemplateRackBased{}, errors.Internal(fmt.Sprintf("template with label %q has unexpected type %T", label, t))
	}
