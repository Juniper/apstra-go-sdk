...
func (o *TwoStageL3ClosClient) SystemNodesFromLinkIds(ctx context.Context, linkIds []ObjectId, nodeRole *enum.SystemNodeRole) ([]ObjectId, error) {
	_, result, err := o.systemNodesFromLinkIds(ctx, linkIds, nodeRole)
	return result, err
}

func (o *TwoStageL3ClosClient) systemNodesFromLinkIds(ctx context.Context, linkIds []ObjectId, nodeRole *enum.SystemNodeRole) (QEQuery, []ObjectId, error) {
	systemQueryAttributes := []QEEAttribute{
		NodeTypeSystem.QEEAttribute(),
		{"name", QEStringVal("n_system")},
	}
	if nodeRole != nil {
		systemQueryAttributes = append(systemQueryAttributes, QEEAttribute{Key: "role", Value: QEStringVal(nodeRole.String())})
	}
...
// is returned.
func (o *TwoStageL3ClosClient) SystemNodeFromLinkIds(ctx context.Context, linkIds []ObjectId, nodeRole *enum.SystemNodeRole) (ObjectId, error) {
