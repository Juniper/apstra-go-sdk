		...
		t.Run(client.Name(), func(t *testing.T) {
			...
			req := apstra.CreateBlueprintFromTemplateRequest{
				...
				FabricSettings: &apstra.FabricSettings{
					FabricL3Mtu:          pointer.To(uint16(rand.Intn(50)*2 + 9100)),
...
func TestCreateDeleteBlueprintWithRoutingLimits(t *testing.T) {
	...
	testCases := []testCase{
		...
		},
		{
			...
			fabricSettings: apstra.FabricSettings{
				MaxEvpnRoutes:     pointer.To(uint32(20001)),
				MaxExternalRoutes: pointer.To(uint32(20002)),
				MaxFabricRoutes:   pointer.To(uint32(20003)),
				MaxMlagRoutes:     pointer.To(uint32(20004)),
			},
		},
	}
