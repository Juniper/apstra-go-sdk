...
func NewStreamTarget(cfg *AosStreamTargetCfg) (*AosStreamTarget, error) {
	...
	return &AosStreamTarget{
		...
		msgChan:   make(chan aosStreaming.Message),
		tlsConfig: tlsConfig,
	}, nil
}
...
type AosStreamTarget struct {
	...
	errChan   chan error
	msgChan   chan aosStreaming.Message
	port      uint16
...
func (o AosStreamTarget) Start() (msgChan <-chan aosStreaming.Message, errChan <-chan error, err error) {
	var nl net.Listener
...
func handleMessages(conn net.Conn, msgChan chan<- aosStreaming.Message, errChan chan<- error) {
	defer conn.Close()
...
func msgFromBytes(in []byte) (*aosStreaming.Message, error) {
	msg := &aosStreaming.Message{}
