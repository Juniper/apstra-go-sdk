		...
		t.Run(tName, func(t *testing.T) {
			...
			for _, client := range clients {
				t.Run(client.Name(), func(t *testing.T) {
					...
					// create the object
					ctx1m, cf := context.WithTimeout(ctx, time.Minute)
					id, err = client.Client.CreateConfigTemplate2(ctx1m, create)
					cf()
					require.NoError(t, err)

					// ensure the object is deleted even if tests fail
					testutils.CleanupWithFreshContext(t, 60, func(ctx context.Context) error {
					...
					ctx1m, cf = context.WithTimeout(ctx, time.Minute)
					obj, err = client.Client.GetConfigTemplate2(ctx1m, id)
					cf()
					require.NoError(t, err)
					idPtr := obj.ID()
					...
					ctx1m, cf = context.WithTimeout(ctx, time.Minute)
					obj, err = client.Client.GetConfigTemplateByLabel2(ctx1m, create.Label)
					cf()
					require.NoError(t, err)
					idPtr = obj.ID()
					require.NotNil(t, idPtr)
					...
					ctx1m, cf = context.WithTimeout(ctx, time.Minute)
					ids, err := client.Client.ListConfigTemplates2(ctx1m)
					cf()
					require.NoError(t, err)
					...
					ctx1m, cf = context.WithTimeout(ctx, time.Minute)
					objs, err := client.Client.GetConfigTemplates2(ctx1m)
					cf()
					require.NoError(t, err)
					...
					ctx1m, cf = context.WithTimeout(ctx, time.Minute)
					err = client.Client.UpdateConfigTemplate2(ctx1m, update)
					cf()
					require.NoError(t, err)
					...
					require.Equal(t, id, *update.ID())
					ctx1m, cf = context.WithTimeout(ctx, time.Minute)
					err = client.Client.UpdateConfigTemplate2(ctx1m, create)
					cf()
					require.NoError(t, err)
					...
					// delete the object
					ctx1m, cf = context.WithTimeout(ctx, time.Minute)
					err = client.Client.DeleteConfigTemplate2(ctx1m, id)
					cf()
					require.NoError(t, err)
					...
					ctx1m, cf = context.WithTimeout(ctx, time.Minute)
					_, err = client.Client.GetConfigTemplate2(ctx1m, id)
					cf()
					require.Error(t, err)
					...
					ctx1m, cf = context.WithTimeout(ctx, time.Minute)
					_, err = client.Client.GetConfigTemplateByLabel2(ctx1m, create.Label)
					cf()
					require.Error(t, err)
					require.ErrorAs(t, err, &ace)
					require.Equal(t, apstra.ErrNotfound, ace.Type())
