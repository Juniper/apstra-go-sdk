		...
		t.Run(tName, func(t *testing.T) {
			for _, client := range clients {
				t.Run(client.Name(), func(t *testing.T) {
					...
					// create the object
					id, err = client.Client.CreateConfiglet2(ctx, tCase.create)
					...
					err = client.Client.UpdateConfiglet2(ctx, tCase.update)
					require.NoError(t, err)
					...
					require.Equal(t, id, *tCase.update.ID())
					err = client.Client.UpdateConfiglet2(ctx, tCase.create)
					require.NoError(t, err)
					...
					_, err = client.Client.GetConfigletByLabel2(ctx, tCase.create.Label)
					require.Error(t, err)
					require.ErrorAs(t, err, &ace)
					require.Equal(t, apstra.ErrNotfound, ace.Type())
